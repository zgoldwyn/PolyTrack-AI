...
if os.path.exists(model_file):
    print(f"Backing up existing model to {model_name}_backup.zip")
    import shutil
    shutil.copy(model_file, f"{model_name}_backup.zip")

print("Creating fresh model for pretraining")
model = PPO(
    "CnnPolicy",
    env,
    verbose=0,
...
for epoch in range(args.epochs):
    ...
    if acc >= 90.0:
        print("Hit 90% accuracy — stopping to avoid overfitting.")
        break
